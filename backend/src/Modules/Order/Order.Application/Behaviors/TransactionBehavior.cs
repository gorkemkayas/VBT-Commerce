...
public class TransactionBehavior<TRequest, TResponse>(IOrderDbContext dbContext)
    ...
    where TRequest : notnull
{
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        // Necessary because Order calls other modules' commands in-process via ISender: without this,
        // a nested call would try to re-open a transaction on a DbContext that's already mid-transaction.
        if (request is not ICommand<TResponse> || typeof(TRequest).Assembly != typeof(IOrderDbContext).Assembly)
            return await next();
