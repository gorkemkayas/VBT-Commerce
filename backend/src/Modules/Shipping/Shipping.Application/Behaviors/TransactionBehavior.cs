...
public class TransactionBehavior<TRequest, TResponse>(IShippingDbContext dbContext)
    ...
    where TRequest : notnull
{
    public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
    {
        ...
        if (request is not ICommand<TResponse> || typeof(TRequest).Assembly != typeof(IShippingDbContext).Assembly)
            return await next();
