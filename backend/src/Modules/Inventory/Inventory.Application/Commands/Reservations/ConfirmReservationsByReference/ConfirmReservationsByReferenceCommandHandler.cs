...
public class ConfirmReservationsByReferenceCommandHandler(IInventoryDbContext dbContext)
    : IRequestHandler<ConfirmReservationsByReferenceCommand, Unit>
{
    public async Task<Unit> Handle(ConfirmReservationsByReferenceCommand request, CancellationToken cancellationToken)
    {
        ...
        // Known reference, but every reservation under it is already confirmed/released — calling
        // confirm again here would silently do nothing, which more likely means caller misuse than
        // a legitimate retry, so it's surfaced instead of swallowed.
        ...
        foreach (var stockItem in stockItems)
        {
            ...
            foreach (var reservationId in reservationIds)
                stockItem.ConfirmReservation(reservationId);
        }
